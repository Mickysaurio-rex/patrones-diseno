...
public class CuadraTrack implements  IElectricos{
    ...
    public void funcionando(int tiempoMarcha) throws InterruptedException {
        for (int i = 0; i < tiempoMarcha ; i=i+5) {
            ...
            Thread.sleep(5000);
            if(bateria<=0){
                System.out.println("CuadraTrack electrico> se quedo sin bateria: estado actual: 0%");
                bateria=0;
                break;
            }
        }
    ...
    public void cargando(int tiempo) {
        ...
        for (int i = 0; i < tiempo; i++) {
            bateria=bateria+2;
            System.out.println("CuadraTrack electrico: cargando bateria ..."+bateria+"%");
            if(bateria>=100){
                System.out.println("CuadraTrack electrico> estado actual: 100%");
                bateria=100;
                break;
            }
        }
